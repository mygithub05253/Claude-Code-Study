...
- 원본은 매우 단순한 유틸리티 스킬이다. 본 해설은 스킬 자체보다 GStack 생태계 전반의 아키텍처와 의존 관계를 보강해 설명했다. "이 스킬을 왜 쓰는가"라는 맥락을 이해하는 것이 핵심 동작보다 중요하기 때문이다.
- 원본에서 언급하는 "전역 vs 벤더 설치" 감지 로직을 표로 정리해 직관적으로 이해할 수 있도록 했다.
- GitHub Actions에서 gstack을 관리하는 방법은 원본에 없는 내용으로, 한국 대학생의 CI/CD 환경에 맞춰 추가했다.
- Windows 경로 처리 주의사항은 이 프로젝트의 개발 환경(Windows 11)에 맞게 추가한 실용적 팁이다.

> 원본: `~/.claude/skills/gstack-upgrade/SKILL.md`
