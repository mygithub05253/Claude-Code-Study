...
- 새 기능을 구현하기 직전
- 버그를 고칠 때 — 버그를 재현하는 실패 테스트를 먼저 만들고, 그걸 통과시키는 방향으로 고친다
- 리팩토링이 안전한지 확신이 필요할 때 (기존 동작을 고정하는 테스트를 먼저 작성)
- 대학생 과제에서 "동작 스펙이 불명확한" 상태로 구현을 시작하려 할 때

반대로, 완전한 프로토타입/스케치 수준의 탐색 코드에는 과할 수 있다. 다만 "이게 탐색인가 실제 구현인가"의 경계는 흐릿하다. 의심스러우면 TDD를 쓰는 편이 낫다.
...
**버그 수정 시의 TDD**: 나중에 "제목에 공백이 양끝에 있는 키워드는 검색되지 않는다"는 버그 제보가 오면, **먼저 이 버그를 재현하는 실패 테스트**(`"  MT  "` 입력 시에도 공지 2개 반환)를 쓰고, 그다음 `keyword.trim()`을 추가한다. 이 순서가 중요하다.
...
- 원본의 Iron Law("실패 테스트 없이 프로덕션 코드 없음")와 Red-Green-Refactor 사이클은 그대로 유지했다.
- 원본에는 "좋은 TDD 예 vs 나쁜 TDD 예"의 구체적 코드 비교가 더 풍부하게 실려 있을 수 있다. 본 해설은 대학생 수준의 Next.js 15 맥락 예제로 압축했다.
- 원본은 Vitest/Jest/특정 언어에 구애받지 않는 원칙 중심이다. 본 해설은 Vitest + TypeScript로 고정해 실제 실행 가능한 예제로 보여 줬다.
- 원본에서 명시되지 않은 세부 컨벤션(예: 테스트 파일 네이밍)은 본 해설에서도 강제하지 않았다. 프로젝트 관례에 맞추면 된다.

> 원본: `~/.claude/skills/test-driven-development/SKILL.md`
