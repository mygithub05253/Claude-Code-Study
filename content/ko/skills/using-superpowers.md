...
- 대화 시작 시점 — 어떤 스킬이 현재 상황에 맞는지 Claude가 판단해야 할 때
- 사용자가 요청을 던졌는데 "이건 어떤 절차로 처리해야 하지?"가 불명확할 때
- 작업 도중 "지금 이 국면에서 어떤 스킬을 꺼내야 맞는 거지?"가 흐릿해질 때
- 여러 스킬이 후보로 떠오를 때 (예: `investigate` vs `systematic-debugging`)

이 스킬은 **다른 스킬을 호출하기 전에 먼저 통과하는 메타 레이어**라고 생각하면 된다.
...
- 원본은 스킬 시스템 전체의 **메타 문서**로, 본 해설에서 다룬 개념 외에도 더 세부적인 우선순위 규칙을 포함할 수 있다. 본 해설은 대학생 사용자가 자주 마주할 상황 위주로 요약했다.
- 원본의 "1% 규칙"과 "Red Flags 표"는 그대로 유지했다.
- 원본에서 언급되는 일부 내부 구현(스킬 메타데이터 포맷, 자동 활성화 조건)은 본 해설에서 생략했다 — 본 프로젝트의 학습 목표는 "스킬 사용자" 관점이기 때문이다.
- 원본의 Rigid/Flexible 스킬 분류는 본 해설에서 요약해 실었다. 구체 스킬 목록은 원본을 참고하자.

> 원본: `~/.claude/skills/using-superpowers/SKILL.md`
