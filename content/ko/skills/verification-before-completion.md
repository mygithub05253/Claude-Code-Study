...
- 작업이 완료됐다고 선언하기 직전 (커밋, PR 생성 전)
- "버그를 수정했습니다"라고 말하기 전
- "테스트가 모두 통과합니다"라고 주장하기 전
- 빌드가 성공한다고 확인해 줬으면 할 때
- 코드 변경 후 실제로 동작하는지 확인 없이 넘어가는 습관을 고치고 싶을 때
...
- 원본은 매우 간결하게 "완료 주장 전 검증 명령어 실행 + 출력 확인 필수, 증거 먼저"라는 단일 원칙으로 구성된다. 본 해설은 이 원칙을 대학생 개발 맥락에서 실제로 어떻게 적용하는지 구체적인 예시와 함께 풀어냈다.
- 원본에서 언급되는 "커밋 또는 PR 생성 전" 조건을 본 해설에서 구체적인 검증 명령어 표로 확장했다.
- 원본은 검증의 대상을 "완료, 수정, 통과"로 명시한다. 본 해설에서는 빌드/테스트/타입/린트/서버 동작 등 Next.js 15 프로젝트에서 실제로 사용하는 검증 방법을 구체적으로 제시했다.
- 이 스킬은 이 프로젝트의 CLAUDE.md에서 "TypeScript strict, any 금지" 원칙 및 "파괴적 작업 승인 필수" 원칙과 함께 코드 품질의 3대 원칙 중 하나로 간주된다.

> 원본: `~/.claude/skills/verification-before-completion/SKILL.md`
