...
- `/freeze`로 특정 디렉토리만 편집 허용했다가, 작업 범위를 확장해야 할 때
- 좁은 범위에서 디버깅을 마치고, 이제 연관된 다른 모듈도 함께 수정해야 할 때
- Guard 모드(`/guard`)를 사용 중이었고, Freeze 레이어만 해제하고 싶을 때
- 세션을 유지한 채로 편집 제약을 없애고 싶을 때
- "unfreeze", "unlock edits", "remove freeze", "allow all edits" 등의 표현으로 요청할 때
...
- 원본 스킬은 Unfreeze의 목적과 사용 시점을 간결하게 설명한다. 본 해설은 Freeze → 작업 → Unfreeze → 확장 작업의 **전체 워크플로우 흐름도**를 추가했다.
- 원본의 "widen edit scope without ending the session" 원칙이 왜 유용한지를, 세션 재시작 비용(컨텍스트 손실)과 연결지어 설명했다.
- Guard 모드에서 Unfreeze가 Careful을 유지한 채 Freeze만 해제한다는 동작을 명시했다. 원본에서 암묵적으로 전제되는 내용이다.
- 대학생 프로젝트에서 흔한 버그(페이지네이션 offset 계산 오류)를 예제로 활용하여 Unfreeze가 실제로 어떤 흐름에서 등장하는지 구체화했다.

> 원본: `~/.claude/skills/unfreeze/SKILL.md`
