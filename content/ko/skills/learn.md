...
- "지금까지 이 프로젝트에서 뭘 배웠지?" 라고 궁금할 때
- 같은 버그가 반복되는 것 같을 때 ("이거 전에도 고쳤던 거 아닌가?")
- 오래된 학습 항목이 더 이상 유효하지 않을 가능성이 있을 때 (라이브러리 업그레이드, 리팩토링 이후)
- 팀원에게 프로젝트 맥락을 공유해야 할 때 (내보내기)
- 새로운 세션을 시작할 때 이전 작업의 패턴과 결정 이유를 빠르게 복기하고 싶을 때
...
```ts
// 학습을 코드 주석으로 활용하는 패턴
// ⚠️ [학습 2026-04-07] revalidatePath 필수 — 빠뜨리면 캐시 문제 발생
...
- 원본은 "gstack" 환경을 전제하며, Conductor 워크스페이스 간 핸드오프(handoff)를 포함한 세션 연속성 관리에 초점을 맞춘다. 본 해설은 단일 Claude Code CLI 세션 기반의 개인 프로젝트 맥락으로 재해석했다.
- 원본에서 학습은 구조화된 데이터 포맷으로 저장되지만, 본 해설에서는 마크다운 메모리 파일(`MEMORY.md`)과 연동하는 방식으로 설명했다.
- "prune stale learnings"의 기준—어떤 항목이 "오래됐는가"—은 원본에서도 Claude의 판단에 위임하는 방식이다. 사용자가 삭제 여부를 최종 승인한다는 점은 동일하다.
- 대학생 맥락에서 Export 기능은 팀 프로젝트 인수인계 문서로도 활용할 수 있다는 점을 추가했다.

> 원본: `~/.claude/skills/learn/SKILL.md`
