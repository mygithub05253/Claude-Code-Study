...
- 원본의 3단계 프로세스(Load → Execute → Complete)와 "When to Stop" 원칙은 그대로 유지했다.
- 원본은 범용 맥락이지만, 본 해설은 Next.js 15 + Supabase 과제 흐름으로 예제를 구체화했다.
- 원본은 다른 스킬(systematic-debugging, test-driven-development 등)과의 통합 지점을 짧게 언급한다. 본 해설에서는 "태스크 검증 실패 시 투입할 스킬"이라는 방식으로 풀어서 설명했다.
- 원본에서 명시되지 않은 세부 정책(예: 태스크 당 커밋을 몇 개로 할지)은 본 해설의 예제에서도 강제하지 않는다. 프로젝트 컨벤션에 맞게 사용하면 된다.

> 원본: `~/.claude/skills/executing-plans/SKILL.md`
