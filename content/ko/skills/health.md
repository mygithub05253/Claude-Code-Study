...
║ tsc          10.0  ✅  에러 0개                      ║
║ eslint        8.0  ⚠️  경고 2개                      ║
║ vitest        6.0  ⚠️  커버리지 38% (목표: 70%)      ║
║ knip          7.0  ⚠️  미사용 export 3개             ║
...
- 원본은 gstack 생태계의 내부 도구 래퍼를 전제한다. 본 해설은 gstack 없이도 `package.json` 스크립트와 표준 도구(`tsc`, `eslint`, `vitest`, `knip`)로 동일한 워크플로우를 구성하는 방법을 설명했다.
- 원본의 "weighted composite 0-10 score" 계산 방식을 구체적인 가중치 표와 예시 수식으로 풀었다.
- 원본에서 언급하는 "dead code detector"를 Next.js 15 TypeScript 프로젝트에 적합한 `knip`으로 구체화했다.
- 트렌드 추적을 위한 JSON 스키마 예시를 추가했다.

> 원본: `~/.claude/skills/health/SKILL.md`
