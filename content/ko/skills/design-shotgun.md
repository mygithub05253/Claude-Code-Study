...
### 4단계: Next.js 15 Server Component에 통합

방향이 결정되면 서버 컴포넌트 버전으로 구현한다.
...
export default async function NoticesPage() {
  ...
  const { data: notices } = await supabase
    .from("notices")
    .select("id, title, author:profiles(name), created_at, view_count, is_pinned, category")
...
- 원본은 gstack 생태계 스킬로, 비교 보드를 실제 UI로 렌더링하는 내부 헬퍼와 연동된다. 본 해설은 그 핵심 개념인 "다중 변형 탐색 → 피드백 수집 → 수렴"을 Next.js 15 + Tailwind CSS 컴포넌트 코드로 구체화했다.
- 원본의 "proactively suggest" 동작 — 사용자가 UI 기능을 묘사할 때 Claude가 능동적으로 이 스킬을 제안한다 — 을 "언제 사용하나요?" 섹션에서 명시적으로 설명했다.
- 3가지 변형(Minimal, Elevated, Colorful Tag)은 한국 대학생이 자주 참고하는 디자인 레퍼런스(클린 앱, 카드 UI, 카테고리 구분)를 기준으로 직접 구성했다.
- 원본은 `design-html` 스킬과의 연계를 암시하며, 본 해설에서는 그 흐름을 명시적으로 도식화했다.

> 원본: `~/.claude/skills/design-shotgun/SKILL.md`
