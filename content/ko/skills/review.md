...
- 작업 브랜치를 main에 올리기 직전 (ship 스킬의 일부로도 호출됨)
- 다른 사람의 PR을 리뷰해야 할 때, 타이포 레벨이 아닌 **설계/보안 레벨** 문제를 찾고 싶을 때
- 리팩토링 직후 "혹시 내가 놓친 부작용 있나?"를 확인할 때
- 자동 테스트로 잡히지 않는 범주(예: SQL 인젝션 위험)를 한 번 더 스캔하고 싶을 때

반대로, 단순 린트/포맷 문제는 이 스킬의 영역이 아니다. ESLint/Prettier 같은 도구로 커버되는 건 별도로 돌린다.
...
- 원본은 gstack 생태계의 스킬로, `ship` 스킬의 일부로 자동 호출될 수도 있다. 본 해설은 독립적으로 사용하는 경우를 중심으로 설명했다.
- 원본의 네 가지 범주(SQL Safety, LLM Trust Boundaries, Conditional Side Effects, Red Flags)는 그대로 유지했다.
- 원본은 영어 코드 예제 중심이지만, 본 해설은 Next.js 15 + Supabase + 한국 대학생 과제 맥락으로 예제를 재작성했다.
- 원본에서 언급되는 특정 프로젝트 컨벤션이나 내부 도구는 본 해설에서 생략했다. "원본에서 명시되지 않은" 세부 사항은 추측하지 않았다.

> 원본: `~/.claude/skills/review/SKILL.md`
