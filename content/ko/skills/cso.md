...
- **흔한 함정 — git 히스토리에 남은 시크릿**: `.env` 파일을 이미 커밋한 뒤 `.gitignore`에 추가해도 git 히스토리에는 남는다. CSO 스킬이 이를 탐지하면 `git filter-repo`로 히스토리 전체를 정리해야 한다. 처음부터 `.env`를 커밋하지 않는 습관이 중요하다.
...
- 원본은 gstack 생태계의 스킬로, 내부 헬퍼(시크릿 스캔 도구, 의존성 DB 조회 등)와 연동된다. 본 해설은 그 개념을 `pnpm audit`, GitHub Actions, Supabase RLS 등 한국 대학생이 실제로 쓰는 스택 기준으로 재해석했다.
- 원본의 "LLM/AI 보안"과 "스킬 공급망 스캔" 항목은 AI 기반 프로젝트와 Claude Code 스킬 사용자 모두에게 해당되므로 핵심 개념 표에 포함했다.
- 원본에서 두 모드(daily/comprehensive)의 confidence gate 수치(8/10, 2/10)를 그대로 인용하고 한국어 맥락에서 실용적 차이를 설명했다.
- STRIDE 위협 모델링 표를 "동아리 공지 게시판" 시나리오에 맞게 직접 작성해 원본 개념을 구체화했다.

> 원본: `~/.claude/skills/cso/SKILL.md`
