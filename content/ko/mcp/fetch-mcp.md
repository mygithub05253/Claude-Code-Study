...
- 공개 REST API 응답 결과를 Claude 컨텍스트에 넣고 싶을 때
- 외부 라이브러리 공식 문서나 GitHub README를 실시간으로 참조하면서 코드를 작성할 때
- API 엔드포인트가 예상대로 응답하는지 Claude와 함께 테스트할 때
- 정기적으로 변경되는 JSON 데이터를 매번 복사하지 않고 자동으로 가져오고 싶을 때
