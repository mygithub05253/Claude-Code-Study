...
→ [1단계: office-hours 에이전트]
    역할: 요구사항을 정리하고 사용자 스토리 작성
    출력: docs/plans/[기능]-requirements.md

→ [2단계: CEO 에이전트]
    역할: 비즈니스 가치, 우선순위, 리스크 검토
    입력: requirements.md
    출력: docs/plans/[기능]-ceo-review.md

→ [3단계: 디자이너 에이전트]
    역할: UX 흐름, 컴포넌트 구조, 접근성 검토
    입력: requirements.md + ceo-review.md
    출력: docs/plans/[기능]-design-review.md

→ [4단계: 엔지니어 에이전트]
    역할: 기술 스택, 구현 복잡도, 의존성 검토
    입력: 모든 이전 문서
    출력: docs/plans/[기능]-eng-review.md

→ [5단계: executing-plans 에이전트]
    역할: 검토된 계획을 실제 코드로 구현
    입력: 모든 계획 문서
    출력: 완성된 코드
```
...
자주 사용하는 파이프라인은 Claude에게 직접 실행을 위임할 수 있다:

```
"공지 구독 알림" 기능에 대해 GStack 파이프라인을 순서대로 실행해줘:
1. office-hours 에이전트 (요구사항)
