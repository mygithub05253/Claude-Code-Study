...
Plan 에이전트 패턴은 설계 탐색 과정을 서브에이전트에 위임해 메인 세션의 컨텍스트를 보호하는 패턴이다.

```mermaid
flowchart TD
    A[사용자: 기능 요청] --> B[메인 세션에서 Plan Agent 실행 요청]
    B --> C[서브에이전트 독립 실행]
    C --> D[요구사항 분석]
    D --> E[기존 코드 탐색]
    E --> F[설계 옵션 검토]
    ...
    I -- 아니오 --> H
    I -- 예 --> J[새 메인 세션 시작]
    J --> K[plan.md를 컨텍스트로 제공]
    K --> L[깨끗한 컨텍스트에서 구현]
```

Claude Code의 컨텍스트 창은 유한하다. 설계 탐색 단계에서 읽는 파일, 나누는 대화, 시도했다 버린 코드 조각들이 모두 컨텍스트를 소모한다. 이 설계 탐색 과정을 서브에이전트에 위임하면 메인 세션은 최종 설계 문서만 받아 깨끗한 상태에서 구현을 시작할 수 있다.
...
4. 결과를 docs/plans/[기능명]-plan.md에 저장
```

### Task 도구 호출 예시 (Claude Code 내부)

Plan Agent 실행 시 Claude Code 내부적으로 다음과 같은 Task 도구가 사용된다:

```text
Task: 공지사항 좋아요 기능 구현 계획서 작성
Description: |
  1. frontend/components/notices/ 탐색
  2. backend/routes/ 구조 파악
  3. prisma/schema.prisma 데이터 모델 분석
  4. writing-plans 형식으로 계획서 작성
  5. docs/plans/like-feature-plan.md에 저장

Output: 계획서 파일 경로 및 요약
```

### 구현 세션 이어받기

계획서 검토 후 새 세션에서 구현을 시작:
...
- [parallel-dispatch](./parallel-dispatch.md) — 여러 기능을 동시에 구현할 때 병렬 에이전트와 결합 가능
- [gstack-roles](./gstack-roles.md) — Plan Agent를 GStack 파이프라인의 첫 단계로 활용
- [writing-plans 스킬](../skills/writing-plans.md) — Plan Agent가 따라야 할 계획 작성 구조 원본
- [executing-plans 스킬](../skills/executing-plans.md) — 계획 작성 후 구현 단계에서 활용
